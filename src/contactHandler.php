...
class ContactManager {
    ...
    public function saveUser($contact) { // for addUser
        $db = @mysqli_select_db($this->connection, $this->config['dbname']) or die(mysqli_error($this->connection));
        $statement = $this->connection->prepare("INSERT INTO $this->tableName2  (firstName, lastName, emailAddress, password, confirm_password) 
        VALUES (?, ?, ?, ?, ?)");
        $statement->bind_param('sssss', $contact->firstName, $contact->lastName, $contact->emailAddress, $contact->password, $contact->confirmpassword);
        $statement->execute();
        //return True;
    }
